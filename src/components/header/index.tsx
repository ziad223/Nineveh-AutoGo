...
const Header: React.FC<HeaderProps> = ({ sliders }) => {
  if (!sliders || sliders.length === 0) return <HeaderSkeleton />;
            ...
            <SwiperSlide key={slide.id}>
              <div className="relative w-full h-full lg:min-h-[500px]  overflow-hidden flex items-center justify-center">
                {slide.image && typeof slide.image === "string" && slide.image.trim() !== "" ? (
                  <img
                    src={getImageUrl(slide.image, "/images/hero-1.webp")}
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center text-center text-white px-4">
                  <h2
                    ...
                    dangerouslySetInnerHTML={{ __html: typeof slide.title === 'string' ? slide.title : "" }}
                  />

                  <div
                    className="text-sm lg:text-lg max-w-[600px] leading-relaxed"
                    dangerouslySetInnerHTML={{ __html: typeof slide.description === 'string' ? slide.description : "" }}
                  />
                </div>
              </div>
            </SwiperSlide>
