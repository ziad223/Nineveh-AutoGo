...
import HowOrderSkeleton from '../skeletons/HowOrderSkeleton';
...
const HowOrder: React.FC<HowOrderProps> = async ({ steps }) => {
  ...
  if (!steps || steps.length === 0) return <HowOrderSkeleton />;
          ...
          {sortedSteps.map((step) => (
            ...
            >
              ...
              <div className="relative">
                <Image
                  src={getImageUrl(step.image, "/images/logo.png")}
                  alt={(step.title && typeof step.title === 'string') ? step.title.replace(/<[^>]+>/g, '') : 'step'} // إزالة وسوم HTML من alt
                  width={110}
                  height={110}
              ...
              <div className="mx-auto text-center">
                <h2
                  ...
                  dangerouslySetInnerHTML={{ __html: typeof step.title === 'string' ? step.title : "" }}
                />
                <div
                  className="text-sm text-[#989898] text-center mt-2"
                  dangerouslySetInnerHTML={{ __html: typeof step.description === 'string' ? step.description : "" }}
                />
              </div>
            </div>
          ))}
